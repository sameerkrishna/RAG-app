...
async function handleUpload(req, res) {
  ...
  try {
    ...
    const allEmbeddings = [];
    const batches = [];
    for (let i = 0; i < chunks.length; i += BATCH_SIZE3) {
      batches.push(chunks.slice(i, i + BATCH_SIZE3));
    }
    const sets = [];
