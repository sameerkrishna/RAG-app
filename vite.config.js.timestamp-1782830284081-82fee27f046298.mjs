...
async function getDocumentFile(req, res) {
  ...
  try {
    ...
    console.log("Seed path exists:", fs.existsSync(path2.join(seedDir, filename)));
    ...
    if (filename) {
      const seedPath2 = path2.join(seedDir, filename);
      if (fs.existsSync(seedPath2)) {
        const stat = fs.statSync(seedPath2);
        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", contentDisposition(filename));
        return fs.createReadStream(seedPath2).pipe(res);
      }
